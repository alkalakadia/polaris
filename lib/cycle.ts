 ...
 *
 * These are ESTIMATES from the user's own averages. PMOS cycles are often
...
export interface CycleHistory {
  ...
  count: number
  periodLengths: number[] // how many days each period lasted
  averagePeriod: number | null
}

/** Cycle lengths + period lengths + regularity learned from logged periods. */
export function cycleHistory(entries: TrackEntry[]): CycleHistory {
  ...
  // How long each period lasted (consecutive flow days from each start).
  const flowDays = new Set(entries.filter((e) => e.flow && e.flow !== "none").map((e) => e.date))
  const periodLengths: number[] = []
  for (const s of starts) {
    let len = 1
    for (let f = addDays(s, 1); flowDays.has(f); f = addDays(f, 1)) len++
...
export function computeCycle(
  ...
  // For the user's CURRENT status, pass false: we don't roll the prediction
  // into a fabricated new cycle when a predicted period hasn't actually been
  // logged — instead we surface "late". Future-date predictions (calendar) and
  // historical mapping keep rollForward = true.
  rollForward: boolean = true
): CycleStatus {
  ...
  const cyclesPassed = rollForward ? Math.floor(daysSince / cycleLength) : 0
...
/**
 * Whether we have enough LOGGED cycles to responsibly predict. PMOS/PCOS cycles
 * are often irregular, so we do NOT guess a next-period date or "cycle day" from
 * a single onboarding estimate — we wait until the user has logged real cycles.
 * Irregular cycles require an extra logged cycle before we'll predict.
 */
export function canPredictCycle(history: CycleHistory, profile: CycleProfile): boolean {
  const need = profile.cycleIrregular ? 3 : 2
  return history.lengths.length >= need
}
...
export function phaseTip(phase: Phase, goal?: Goal): string {
  ...
  const byGoalNudge: Partial<Record<Goal, Partial<Record<Phase, string>>>> = {
    ...
    regulate: {
      luteal: "Tracking when your period actually arrives is how MyPMOS learns your real rhythm.",
    },
    weight: {
