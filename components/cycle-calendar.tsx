...
export function CycleCalendar({
  ...
  function info(date: Date) {
    ...
    let fertile = false
    if (onboarded && anchor && isFuture && showPredictions) {
  ...
  return (
    <section className="rounded-3xl border border-g-border bg-white p-4 shadow-girly">
        ...
        {cells.map((date, i) => {
          ...
          return (
            ...
            >
              ...
              {shaveDays?.has(d.key) && <span className="absolute -right-0.5 -top-0.5 text-[0.6rem] leading-none">🪒</span>}
            </button>
          )
        })}
      </div>
      ...
      <div className="mt-3 flex flex-wrap gap-x-3 gap-y-1 text-[0.65rem] font-bold text-g-ink-3">
        ...
        <Legend className="ring-2 ring-g-ink" label="Today" />
        <span className="inline-flex items-center gap-1">🪒 Shaved</span>
      </div>
