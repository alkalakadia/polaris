    ...
    <PatientShell>
      <div className="flex items-center gap-3">
        ...
        <span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-candy-soft text-current">
          <MedicalIcon name="photos" size={22} className="text-current" />
        </span>
        <div>
          <h1 className="font-cute text-2xl text-g-ink">Symptom photos</h1>
          <p className="text-sm font-medium text-g-ink-3">Track hair, skin, and acne changes over time.</p>
        </div>
      </div>
      ...
      <section className="mt-4 rounded-3xl border border-g-border bg-white p-4 shadow-girly">
        ...
        {user ? (
          <>
            ...
            <label className="mt-2 inline-flex cursor-pointer items-center gap-2 rounded-full bg-candy-soft px-3.5 py-2 text-sm font-bold text-g-pink-deep active:scale-95">
              {busy ? (
                "Uploading…"
              ) : (
                <>
                  <MedicalIcon name="camera" size={14} className="text-current" />
                  Add a photo ({area})
                </>
              )}
                ...
                {photos.map((p) => (
                  <div key={p.id} className="relative overflow-hidden rounded-2xl border border-g-border bg-g-canvas">
                    {urls[p.id] ? (
                      ...
                      <div className="grid h-40 w-full place-items-center text-g-ink-3">
                        <MedicalIcon name="camera" size={24} className="text-g-ink-3" />
                      </div>
                    )}
                    ...
                    <button
                      ...
                      className="absolute right-1 top-1 grid h-6 w-6 place-items-center rounded-full bg-white/90 text-g-ink shadow"
                      aria-label="Remove"
                    >
      ...
      >
        <span className="inline-flex items-center gap-2">
          <MedicalIcon name="checklist" size={14} className="text-current" />
          Do the Ferriman-Gallwey hair self-score
        </span>
