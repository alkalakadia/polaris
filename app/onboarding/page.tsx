...
export default function OnboardingPage() {
  ...
  function finish() {
    saveProfile({ ...p, completedAt: Date.now() })
    router.push("/")
  }
  ...
  return (
    <div className="bg-girly font-round flex min-h-screen flex-col text-g-ink">
      ...
      <main className="mx-auto flex w-full max-w-md flex-1 flex-col px-5 pt-6">
        {step === 0 && (
          <Step emoji="🌸" title="Hi! What should we call you?" sub="So Polaris feels like yours.">
            <input
              autoFocus
              value={p.name ?? ""}
        ...
        {step === 1 && (
          <Step emoji="🩷" title="When did your last period start?" sub="A rough date is totally fine.">
            ...
            <p className="mt-3 text-xs font-semibold text-g-ink-3">
              PCOS cycles can be irregular — Polaris keeps it gentle and updates as you log.
            </p>
          </Step>
        )}
        ...
        {step === 4 && (
          <Step emoji="💫" title="What's your main goal right now?" sub="We'll tailor your tips and insights to this.">
              ...
              {GOALS.map((g) => (
                <button
                  ...
                  className={cn(
                    ...
                    p.goal === g.id ? "border-transparent bg-candy text-white shadow-girly-pop" : "border-g-border bg-white"
                  )}
                >
                  <span className="text-2xl">{g.emoji}</span>
                  <span>
                    <span className="block font-cute text-base font-bold">{g.label}</span>
                    <span className={cn("text-sm font-semibold", p.goal === g.id ? "text-white/90" : "text-g-ink-3")}>
                      {g.blurb}
                    </span>
                  </span>
                </button>
