...
import { shaveStats } from "@/lib/hirsutism"
import { concernLabel, contraSummary, getHistory } from "@/lib/history"
...
function loadConfig(): ExportConfig {
  const base: ExportConfig = {
    ...
    chiefConcern: "",
    tried: "",
    questions: ["Could my symptoms be PMOS-related?", "Should I get bloodwork done?"],
    notes: "",
  }
...
export default function ExportPage() {
  ...
  const [assessments, setAssessments] = useState<Partial<Record<string, AssessmentResult>>>({})
  const [labs, setLabs] = useState<LabEntry[]>([])
  const { user } = useAuth()

  useEffect(() => {
    setGoals(getGoals())
    setAssessments(getAllResults())
    setLabs(getLabs())
  }, [])
  ...
  const GOAL_SECTIONS: Record<string, string[]> = {
    regulate: ["cycle", "summary"],
    symptoms: ["symptoms", "watchouts"],
    ttc: ["cycle"],
    weight: ["summary"],
    understand: ["summary", "symptoms"],
  }
  ...
  const androgenLabIds = new Set(["totalT", "freeT", "shbg", "dheas", "androstenedione"])
  const preVisit = buildPreVisit(profile, entries, {
    fgTotal: fg ? fg.score : null,
    fgCutoff: fg ? fgCutoff(fg.meta?.ethnicity ?? "general") : null,
    acneSeverity: assessments.acne ? assessments.acne.score : null,
    hairLossSeverity: assessments.hairloss ? assessments.hairloss.score : null,
    hasAndrogenLab: labs.some((l) => androgenLabIds.has(l.labId)),
    hasAMH: labs.some((l) => l.labId === "amh"),
  })
  const screenerResults = SCREENERS.map((s) => ({ name: s.name, r: assessments[s.id] })).filter((x) => x.r)
  const shaves = shaveStats()
  const hist = getHistory()
  const mainConcern = hist.mainConcern ? concernLabel(hist.mainConcern.id) : null
  const hasPreVisit =
    preVisit.signalCount > 0 ||
    !!fg ||
  ...
  const snapshot = [
    ...
    profile.lastPeriodStart
      ...
      : null,
    focusGoalLabels.length ? `focus: ${focusGoalLabels.join(", ")}` : null,
      ...
      <div className="print-area mt-6 overflow-hidden rounded-3xl border border-g-border bg-white shadow-girly">
        ...
        <div className="space-y-5 p-6">
          ...
          <section className="rounded-2xl border-2 p-4" style={{ borderColor: theme.to }}>
            ...
            {focusGoalLabels.length > 0 && (
              <PreItem label="What I most want help with">{focusGoalLabels.join(" · ")}</PreItem>
            )}
