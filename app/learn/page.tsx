...
export default function LearnPage() {
  ...
  async function loadResearch(prof: CycleProfile, force: boolean) {
    ...
    try {
      ...
      setResError("Couldn't reach Polaris. Try again?")
    } finally {
      setResLoading(false)
    }
  }
  ...
  async function ask(question: string) {
    ...
    try {
      ...
      setAskError("Couldn't reach Polaris. Try again?")
    } finally {
      setAsking(false)
    }
  }
    ...
    <PatientShell>
      ...
      {(asking || answer || askError) && (
        <div className="mt-3 rounded-3xl border border-g-border bg-white p-5 shadow-girly">
          {asking ? (
            <p className="text-sm font-bold text-g-ink-3">Polaris is thinking… 🌸</p>
          ) : askError ? (
