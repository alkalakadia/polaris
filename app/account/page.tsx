...
export default function AccountPage() {
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault()
    if (mode === "up") {
      if (!ageOk) return setError("Please confirm you're 13 or older (with a parent's help if you're a minor).")
      ...
      <form onSubmit={submit} className="mt-4 space-y-3">
        ...
        {mode === "up" && (
          <div className="space-y-2 rounded-2xl bg-g-canvas px-4 py-3">
            <label className="flex items-start gap-2.5 text-xs font-semibold text-g-ink-2">
              <input type="checkbox" checked={ageOk} onChange={(e) => setAgeOk(e.target.checked)} className="mt-0.5 h-4 w-4 shrink-0 accent-[var(--g-pink)]" />
              <span>I&apos;m 13 or older. If I&apos;m a minor, I&apos;m using MyPMOS with a parent or guardian.</span>
            </label>
            <label className="flex items-start gap-2.5 text-xs font-semibold text-g-ink-2">
              <input type="checkbox" checked={agreed} onChange={(e) => setAgreed(e.target.checked)} className="mt-0.5 h-4 w-4 shrink-0 accent-[var(--g-pink)]" />
              <span>
                I agree to the{" "}
                <a href="/terms" className="font-bold text-g-pink-deep underline">Terms</a> and{" "}
                <a href="/privacy" className="font-bold text-g-pink-deep underline">Privacy Policy</a>, and I understand
                MyPMOS is not medical care and does not diagnose.
              </span>
            </label>
          </div>
        )}
...
function AccountHub({ user }: { user: User }) {
  ...
  async function deleteAccount() {
    ...
    if (!window.confirm("Really delete everything — tracker, labs, photos, posts, and account? There's no going back.")) return
    setDeleting(true)
    try {
      ...
      if (token) {
        ...
        if (!res.ok) {
          setDeleting(false)
          window.alert("Something went wrong deleting your account. Please try again or email hello@polaris.app.")
          return
        }
      }
      try {
  ...
  return (
    <>
      ...
      {/* My health data */}
      <h2 className="mt-6 px-1 font-cute text-base font-bold text-g-ink">My health</h2>
      <div className="mt-2 overflow-hidden rounded-3xl border border-g-border bg-white shadow-girly">
        <SettingRow emoji="🩸" label="Period tracker" href="/period" />
        <SettingRow emoji="🎯" label="My goals" href="/goals" />
        <SettingRow emoji="📋" label="Self-checks (hair, mood, sleep)" href="/assessments" />
        <SettingRow emoji="🪶" label="Symptom photos (hair, skin, acne)" href="/hirsutism" />
        <SettingRow emoji="🧪" label="My labs" href="/labs" />
        <SettingRow emoji="⌚️" label="Activity & import (steps, sleep…)" href="/import" />
        <SettingRow emoji="📝" label="My symptom profile" href="/symptoms" />
        <SettingRow emoji="🩺" label="My health profile" href="/health" />
        <SettingRow emoji="🗂️" label="My history (concern + birth control)" href="/history" />
      </div>

      {/* For your doctor */}
      <h2 className="mt-5 px-1 font-cute text-base font-bold text-g-ink">For your doctor</h2>
      <div className="mt-2 overflow-hidden rounded-3xl border border-g-border bg-white shadow-girly">
        <SettingRow emoji="📄" label="Pre-visit summary / Gyno PDF" href="/export" />
        <SettingRow emoji="🩺" label="Find a provider" href="/providers" />
      </div>

      {/* Learn & connect */}
      <h2 className="mt-5 px-1 font-cute text-base font-bold text-g-ink">Learn & connect</h2>
      <div className="mt-2 overflow-hidden rounded-3xl border border-g-border bg-white shadow-girly">
      ...
      {/* Account */}
      <h2 className="mt-5 px-1 font-cute text-base font-bold text-g-ink">Account</h2>
      <div className="mt-2 overflow-hidden rounded-3xl border border-g-border bg-white shadow-girly">
        ...
        <SettingRow
          emoji="🗑️"
          label={deleting ? "Deleting…" : "Delete my account & data"}
      ...
      {/* Legal */}
      <h2 className="mt-5 px-1 font-cute text-base font-bold text-g-ink">Legal & privacy</h2>
      <div className="mt-2 overflow-hidden rounded-3xl border border-g-border bg-white shadow-girly">
        <SettingRow emoji="🔒" label="Privacy Policy" href="/privacy" />
        <SettingRow emoji="📜" label="Terms of Use" href="/terms" />
      </div>
