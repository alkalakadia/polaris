...
export default async function RealHandoutPage({
  ...
  const firstName = (intake.intake_data["first_name"] as string) || "Hi there"

  const patientLike: SamplePatient = adaptIntakeToPatient(
    intake.id,
    firstName,
    intake.intake_data,
      ...
      <header className="sticky top-0 z-10 border-b border-border-soft bg-canvas-tint/80 backdrop-blur print:hidden">
        <div className="mx-auto flex max-w-3xl items-center justify-between px-6 py-3">
          <div className="flex items-center gap-3">
            ...
            <span
              className={`rounded-full px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider ${
                aiPowered ? "bg-sage/15 text-sage" : "bg-peach-soft text-coral-deep"
              }`}
            >
              {aiPowered
          ...
          <div className="flex items-center gap-2">
            <a
              href={`mailto:?subject=Your visit summary&body=Hi ${firstName}, attached is your visit summary.`}
